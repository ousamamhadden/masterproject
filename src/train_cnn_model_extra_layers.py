...
class CNNClassLabels(nn.Module):
    def __init__(self, filter_size=filter_size, padding=padding, num_classes=len(labels)):
        ...
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=filter_size, stride=1, padding=padding)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.fc1 = nn.Linear(64 * 27 * 18, 256)  # 32 channels of size 27 * 18 (due to 216x144 being pooled thrice)
    ...
    def forward(self, x):
        ...
        x = F.relu(x)              # Apply activation
        x = self.pool(x)           # Apply pooling
        x = x.view(-1, 64 * 27 * 18)  # Reshape to flat vector for fully connected layer
        x = self.fc1(x)            # Apply fully connected layer
        x = F.relu(x)              # Apply activation
        x = self.fc2(x)            # Apply final fully connected layer (no activation for regression)
        return x
...
for epoch in range(num_epochs):
    ...
    torch.save(model.state_dict(), os.path.join(models_dir, f"cnn_classlabels_extra_layers_e{epoch+1}_d{dt_string}_s{len(train_dataset)}"))
